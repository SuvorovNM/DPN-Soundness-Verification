...
public static class SoundnessAnalyzer
{
    ...
    public static Dictionary<AbstractState, ConstraintStateType> GetStatesDividedByTypesNew<AbsState, AbsTransition,
            AbsArc>
        ...
        where AbsArc : AbstractArc<AbsState, AbsTransition>
    {
        ...
        void DefineStatesWithNoWayToFinals(Dictionary<AbstractState, ConstraintStateType> stateDictionary,
            IEnumerable<AbsState> finalStates)
        ...
        static void DefineUncleanFinals(IEnumerable<Place> terminalNodes,
            Dictionary<AbstractState, ConstraintStateType> stateDictionary, IEnumerable<AbsState> finalStates)
        {
            finalStates
    ...
    public static Dictionary<StateType, List<LtsState>> GetStatesDividedByTypes(ConstraintGraph graph,
        IEnumerable<Place> terminalNodes)
    {
        ...
        static void FillDeadlocks(ConstraintGraph graph, IEnumerable<Place> terminalNodes,
            Dictionary<StateType, List<LtsState>> stateDict)
        {
        ...
        static void FillUncleanFinals(ConstraintGraph graph, IEnumerable<Place> terminalNodes,
            Dictionary<StateType, List<LtsState>> stateDict)
        {
            stateDict[StateType.UncleanFinal] = graph.ConstraintStates
                .Where(x => x.Marking.Keys.Intersect(terminalNodes).All(y => x.Marking[y] > 0)
                            && (x.Marking.Keys.Any(y => x.Marking[y] > 1) ||
                                x.Marking.Keys.Except(terminalNodes).Any(y => x.Marking[y] > 0)))
                .ToList();
        }

        static void FillCleanFinals(ConstraintGraph graph, IEnumerable<Place> terminalNodes,
            Dictionary<StateType, List<LtsState>> stateDict)
        {
        ...
        static void FillSoundIntermediate(ConstraintGraph graph, Dictionary<StateType, List<LtsState>> stateDict)
        {
            stateDict[StateType.SoundIntermediate] = graph.ConstraintStates
                ...
                .Except(stateDict[StateType.NoWayToFinalMarking])
                .ToList();
        }
    }
}